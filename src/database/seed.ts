...
async function runSeed() {
  ...
  console.log('🌱 Iniciando siembra de datos agrícolas...');
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    for (let j = 0; j < batchSize; j++) {
      batch.push(
        productRepo.create({
          // Nombres que suenan a inventario real
          ...
          price: parseFloat(faker.commerce.price({ min: 50, max: 5000 })), // Precios más acordes a insumos
  ...
  console.log('✅ ¡Siembra completada! 50,000 registros listos para administrar sin Excel.');
  await app.close();
}
